...
case $codename in
  "buster")
  echo "Adding adoptopenjdk repository" >> "${SEEDIT_LOG}"  2>&1
  apt-get -y -q install software-properties-common >> "${SEEDIT_LOG}"  2>&1
  wget -qO- https://adoptopenjdk.jfrog.io/adoptopenjdk/api/gpg/key/public | apt-key --keyring /etc/apt/trusted.gpg.d/adoptopenjdk.gpg add - >> "${SEEDIT_LOG}"  2>&1
  add-apt-repository --yes https://adoptopenjdk.jfrog.io/adoptopenjdk/deb/ >> "${SEEDIT_LOG}"  2>&1
  apt-get -y -q update >> "${SEEDIT_LOG}"  2>&1
  apt-get -y -q install adoptopenjdk-8-hotspot >> "${SEEDIT_LOG}"  2>&1 || { echo "ERROR: Could not download Java. Exiting." >> "${SEEDIT_LOG}"  2>&1; exit 1; }
  ;;
  *)
  apt-get -y install openjdk-8-jre || { echo "ERROR: Could not download Java. Exiting." >> "${SEEDIT_LOG}"  2>&1; exit 1; }
  ;;
esac
...
dpkg -i subsonic.deb >> "${SEEDIT_LOG}"  2>&1
...
systemctl enable --now subsonic.service >> "${SEEDIT_LOG}"  2>&1

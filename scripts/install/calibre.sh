...
_install() {
    apt_install xdg-utils wget xz-utils libxcb-xinerama0 libfontconfig libegl1 libgl1-mesa-glx libopengl0
    ...
    if [[ $(_os_arch) = "amd64" ]]; then
        ...
        sed -i "s|def check_glibc_version.*|def check_glibc_version(min_required=(2, 11), release_date='2020-02-01'):|g" /tmp/calibre-installer.sh
